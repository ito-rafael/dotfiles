...
class AnsibleProvisionApp(Gtk.ApplicationWindow):
    ...
    def __init__(self, app):
        ...
        grid.set_halign(Gtk.Align.CENTER) # Centers the tight form in the window
        ...
        # Spanning 4 columns forces the header to match the width of the form inputs!
        grid.attach(header_box, 0, 0, 4, 1)
        ...
        # vertical separator spans Rows 1 to 4
        v_sep = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        v_sep.set_halign(Gtk.Align.CENTER) # Prevents the rectangle! Keeps it 1px wide in the center.
        v_sep.set_hexpand(True) # Makes this column act as a spring to absorb empty space.
        ...
        grid.attach(task_box, 0, 5, 4, 1)

        # horizontal separator - Row 6
        h_sep = Gtk.Separator(margin_top=5, margin_bottom=5)
        grid.attach(h_sep, 0, 6, 4, 1)
        ...
        grid.attach(cmd_box, 0, 7, 4, 1)

        # left: Mode Indicator - Row 8
