...
where:
    ...
    -ws          workspace number to navigate to (can be a list of workspaces separated with space)"
#------------------------
# print help menu
if [[ $1 == '-h' || $1 == '--help' ]]; then
...
# parse arguments
    ...
    for ws in $@; do
        ...
        if ! [[ $ws =~ ^[0-9]{,2}$ ]]; then
            printf "Error: 'ws' parameter '$ws' missing or not understood!\n\n"
            echo "$usage"
            exit
        else
            WORKSPACE=$ws
            ...
            if ${CMD_MSG} -t get_workspaces | grep -e "$WS_NAME"; then
