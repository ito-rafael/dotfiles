...
Please, refer to these documentations whenever needed:
  ...
  - [[https://github.com/ito-rafael/dotfiles/blob/master/archlinux/installation-btrfs/installation-btrfs.org][My personal guide for installing Arch with BTRFS filesystem]]
  - [[https://github.com/ito-rafael/dotfiles/blob/master/archlinux/installation-ext4/installation-ext4.org][My personal guide for installing Arch with ext4 filesystem]] (deprecated)
...
Don't forget to set a strong password for the root (and for the regular user, if you set one now), before proceeding with the installation.
#+begin_src sh
archinstall \
    --config-url https://raw.githubusercontent.com/ito-rafael/dotfiles/refs/heads/master/archlinux/archinstall/config.json \
    --creds-url https://raw.githubusercontent.com/ito-rafael/dotfiles/refs/heads/master/archlinux/archinstall/credentials.json
...
#+end_src
*** Full configuration
After validating the installation, I reboot and then use [[https://docs.ansible.com/][Ansible]] to customize my full system. Please check my [[https://github.com/ito-rafael/ansible-provision][ansible-provision]] Git repository for more information.
#+begin_src sh
...
#+end_src
* User configuration
#+begin_src js :noweb-ref bos
{
#+end_src
** Audio
#+begin_src js :noweb-ref audio
"app_config": {
  "audio_config": {
    "audio": "pipewire"
  }
...
#+end_src
** Bootloader
#+begin_src js :noweb-ref bootloader
...
:PROPERTIES:
:header-args:  :noweb-ref post-installation
:END:
#+begin_src js
"custom_commands": [
#+end_src
...
"reflector --country <<vars-general("country")>> --age 6 --sort rate --save /etc/pacman.d/mirrorlist",
#+end_src
*** Fix boot directory
...
Ensure live boot hostname (=archiso=) is updated, in order to run [[https://github.com/ito-rafael/ansible-provision][ansible-provision]].
#+begin_src js
...
#+begin_src js :noweb yes
{
  "btrfs": [
    ...
    {
      "mountpoint": "/home/<<vars-general("username")>>/.cache",
      "name": "@cache"
    },
    {
      "mountpoint": "/var/log",
...
#+end_src
** Hostname
#+begin_src js :noweb yes :noweb-ref hostname
"hostname": "<<vars-general("hostname")>>",
#+end_src
** Kernel
#+begin_src js :noweb yes :noweb-ref kernel
"kernels": [
  "<<vars-general("kernel")>>"
],
#+end_src
** Locale
#+begin_src js :noweb-ref locale
"locale_config": {
  "kb_layout": "us",
  "sys_enc": "UTF-8",
  "sys_lang": "en_US"
...
#+end_src
** NTP
#+begin_src js :noweb-ref ntp
"ntp": true,
#+end_src
** Packages
#+begin_src js :noweb-ref packages
"packages": [
    "base-devel",
    "linux-firmware",
    "btrfs-progs",
...
#+end_src
** Miscellaneous
#+begin_src js :noweb yes :noweb-ref misc
"parallel downloads": 5,
"script": null,
"services": [],
"swap": true,
"timezone": "<<vars-general("timezone")>>",
"silent": true,
"debug": false,
"version": "3.0.11",
#+end_src
** Network
*** DHCP
#+begin_src js :noweb-ref network-dhcp
"network_config": {
  "type": "nm"
}
#+end_src
*** Static IP
#+begin_src js :noweb yes :noweb-ref network-static-ip
"network_config": {
  "type": "manual",
  "nics": [
    {
      ...
      "ip": "<<vars-network("ip")>>",
      "dhcp": <<vars-network("dhcp")>>,
      "gateway": "<<vars-network("gateway")>>",
      "dns": <<vars-network("dns")>>
    }
  ]
}
#+end_src
...
* Tangle :noexport:
** DHCP
#+begin_src js :noweb yes :tangle config.json
<<bos>>
<<audio>>
<<bootloader>>
<<post-installation>>
...
#+end_src
** Static IP
#+begin_src js :noweb yes :tangle config_static-ip.json
<<bos>>
<<audio>>
<<bootloader>>
<<post-installation>>
