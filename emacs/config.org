...
((string-match-p "/\\(sway\\|scroll\\)/" target-dest)
 (let* ((runtime-dir (format "/run/user/%s" (user-uid)))
        (sway-socket
         (car (file-expand-wildcards
               (expand-file-name "sway-ipc.*.sock" runtime-dir))))
        (scroll-socket
         (car (file-expand-wildcards
               (expand-file-name "scroll-ipc.*.sock" runtime-dir)))))
   ...
   (cond
    ...
    ;;------------------------
    (sway-socket
     (let* ((wm-dir (expand-file-name "~/.config/sway/"))
            (script-path (expand-file-name "i3bang.rb" wm-dir))
            (config-source (expand-file-name "_config" wm-dir))
            (cmd (format
                  "export SWAY_SCALE=$(~/.config/scripts/sway-scale.sh get); \
ruby '%s' '%s' && \
swaymsg -s '%s' reload && \
sleep 1 && \
~/.config/scripts/sway-scale.sh \"$SWAY_SCALE\""
                  script-path
                  config-source
                  sway-socket)))
    ...
    ;;------------------------
    (scroll-socket
     (let* ((wm-dir (expand-file-name "~/.config/scroll/"))
            (script-path (expand-file-name "i3bang.rb" wm-dir))
            (config-source (expand-file-name "_config" wm-dir))
            (cmd (format
                  "export SWAY_SCALE=$(~/.config/scripts/sway-scale.sh get); \
ruby '%s' '%s' && \
scrollmsg -s '%s' reload && \
sleep 1 && \
~/.config/scripts/sway-scale.sh \"$SWAY_SCALE\""
                  script-path
                  config-source
                  scroll-socket)))
       (call-process-shell-command cmd nil "*Messages*")
       (message "↻ Generated & Reloaded Scroll.")))

    ;;------------------------
    ;; neither compositor is running
    ;;------------------------
    (t
     (message "No Sway or Scroll IPC socket found; skipping reload.")))))
#+end_src
***** xremap
#+begin_src emacs-lisp
