...
(use-package frame
  ...
  (window-divider-default-bottom-width 15)  ;; bottom gaps width (in pixels)
  ...
  ;; get the background color of the currently loaded theme
...
*** Copy
#+begin_src emacs-lisp
;; send text to wl-copy with strict UTF-8 encoding
(setq interprogram-cut-function
  (lambda (text &optional push)
    (let ((process-connection-type nil)
          ;; force Emacs to output UTF-8 bytes (instead of Latin-1)
          (coding-system-for-write 'utf-8))
      ;; send to wl-copy and explicitly label it as UTF-8
      (start-process "wl-copy" nil "wl-copy" "--type" "text/plain;charset=utf-8" text))))
#+end_src
*** Paste
#+begin_src emacs-lisp
...
(use-package doom-themes
  ...
  (defun efs/sync-window-borders ()
    "Update window divider (internal/external) gaps colors to match the theme."
    (let ((bg-color (face-attribute 'default :background)))
      (set-face-attribute 'window-divider nil :foreground bg-color)
      (set-face-attribute 'internal-border nil :background bg-color)))
  ;; run this function immediately
  (efs/sync-window-borders)
  ;; run it automatically whenever a Doom theme is loaded
...
(use-package org-auto-tangle
  ...
  :custom
  (org-auto-tangle-babel-safelist '(all))
...
(let ((transfer-list `(
  ...
  ;; keebab
  (,(expand-file-name "~/git/keebab/xremap/config.org")         . (("config_sway.yml"  . "~/.config/xremap/config")))
...
#+begin_src emacs-lisp
  (efs/leader-keys
    ...
    "t b" '(lambda () (interactive) (dashboard-open) (dashboard-refresh-buffer) (dashboard-jump-to-recents)  :wk "Toggle dashboard")
