...
(setq native-comp-async-report-warnings-errors 'silent)  ;; silence native-compilation warnings from 3rd party packages
(setq warning-minimum-level :error)                      ;; only pop up for actual errors (no warnings)
#+end_src
** Bars
#+begin_src emacs-lisp
...
(global-visual-line-mode 1)                       ;; enable text wrapping
#+end_src
*** Exceptions
...
#+end_src
** Splits control
Give preference for vertical splits (side-by-side).
#+begin_src emacs-lisp
...
*** Paste
#+begin_src emacs-lisp
;; read from wl-paste, comparing against the yanked text
(setq interprogram-paste-function
  (lambda ()
    ;; set default-directory to local home dir (fix paste while in TRAMP)
...
;; bypass sway-dispatch focus_output command
(global-set-key (kbd "<0x100811d2>") 'ignore)
#+end_src
** Debug
#+begin_src emacs-lisp
...
(defun efs/warp-mouse-to-center (&rest _)
  ...
  (when (display-graphic-p)
    (let* ((edges (window-pixel-edges))
           ...
           (rel-x  (+ left (/ (- right left) 2)))
           (rel-y  (+ top (/ (- bottom top) 2)))
           ;; get x/y offsets of the current monitor
           (jq-cmd "swaymsg -t get_outputs | jq -r '.[] | select(.focused) | .rect | [ .x, .y ] | join(\" \")'")
           (offset-str (shell-command-to-string jq-cmd))
           (offset-list (split-string offset-str))
           ;; convert to lisp numbers
           (offset-x (if (nth 0 offset-list) (string-to-number (nth 0 offset-list)) 0))
           (offset-y (if (nth 1 offset-list) (string-to-number (nth 1 offset-list)) 0))
           ;; calculate the absolute coordinates (relative + offset + 1920)
           (abs-x (+ offset-x rel-x))
           (abs-y (+ offset-y rel-y))
           ;; warp the cursor
           (cmd (format "swaymsg seat seat0 cursor set %d %d" abs-x abs-y)))
      ;; the output for "swaymsg -t get_outputs | jq -r '.[] | select(.focused).rect.x'" is:
      ;;   - left monitor: x=-1920
      ;;   - central monitor: x=0
      ;;   - right monitor: x=1920
      (start-process-shell-command
       (format "Moving-mouse-to-x=%d-y=%d" abs-x abs-y)
       "*Messages*"
       cmd))))
...
(use-package evil
  ...
  (setq evil-want-minibuffer nil)     ;; disable evil in minibuffer
  :custom
  ...
  (evil-define-key nil evil-insert-state-map
    ...
    (kbd "<f13> v") (lambda () (interactive) (insert "'v "))
    ; misc upperrcase
    (kbd "<f13> N") (lambda () (interactive) (insert "'N "))
    (kbd "<f13> M") (lambda () (interactive) (insert "'M "))
    (kbd "<f13> R") (lambda () (interactive) (insert "'R "))
...
(use-package evil-snipe
  ...
  (evil-snipe-mode 1)
  ; do not use snipe engine search with {f,F,t,T}
  (evil-snipe-override-mode 0)

  ;; unbind keys from minor mode map to avoid conflicts with Colemak-DH translations
  (evil-define-key '(normal motion visual) evil-snipe-local-mode-map
    "s" nil
    "S" nil
    "x" nil
    "X" nil)

  ;; bind {f,F,t,T} use snipe's 2-char functions globally
  (evil-global-set-key 'motion "f" #'evil-snipe-s)   ;; 2-char inclusive forward
  (evil-global-set-key 'motion "F" #'evil-snipe-S)   ;; 2-char inclusive backward
  (evil-global-set-key 'motion "t" #'evil-snipe-x)   ;; 2-char exclusive forward
  (evil-global-set-key 'motion "T" #'evil-snipe-X)   ;; 2-char exclusive backward
)
#+end_src
* Theme
** doom-themes
...
(use-package doom-modeline
  :custom
  (doom-modeline-height 15)   ;; adjust modeline height
  (doom-modeline-bar-width 5) ;; colored bar on the left
  :init
  (doom-modeline-mode 1)      ;; enable modeline
)
#+end_src
* Org
#+begin_src emacs-lisp
(use-package org
  ...
  ;; map #+begin_src {tofu,hcl} to use terraform-mode
  (with-eval-after-load 'org
    (add-to-list 'org-src-lang-modes '("tofu" . terraform))
    (add-to-list 'org-src-lang-modes '("hcl" . terraform))
    (add-to-list 'org-src-lang-modes '("terraform" . terraform)))
)
#+end_src
** Filters for ":tangle no"
*** Gray ":tangle no" blocks
#+begin_src emacs-lisp
;; make ":tangle no" blocks appear grayed-out (like comments)
(defun efs/org-gray-out-tangle-no-blocks (orig-fun lang start end)
  "Advice to gray out blocks with :tangle no instead of native syntax highlighting."
  (let ((is-tangle-no nil)
        (case-fold-search t))
    (save-excursion
      (goto-char start)
      (beginning-of-line)
      (let* ((line-current (buffer-substring-no-properties (line-beginning-position) (line-end-position)))
             (line-previous (progn (forward-line -1)
                                   (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
             (regex-begin "^[ \t]*#\\+begin_src")
             (regex-tangle ":tangle[ \t]+no\\b"))
        (if (and (string-match-p regex-begin line-current)
                 (string-match-p regex-tangle line-current))
            (setq is-tangle-no t)
          (when (and (string-match-p regex-begin line-previous)
                     (string-match-p regex-tangle line-previous))
            (setq is-tangle-no t)))))

    ;; when the tag ":tangle no" is present
    (if is-tangle-no
        (progn
          ;; prepend the comment face so it visually overrides old syntax highlights
          ;; loop through property changes to avoid infinite stacking
          (let ((pos start))
            (while (< pos end)
              (let ((next (next-single-property-change pos 'font-lock-face nil end))
                    (face (get-text-property pos 'font-lock-face)))
                (unless (if (listp face) (memq 'font-lock-comment-face face) (eq face 'font-lock-comment-face))
                  (font-lock-prepend-text-property pos next 'font-lock-face 'font-lock-comment-face))
                (setq pos next))))
          t) ;; return t to skip native fontification

      ;; when the tag ":tangle no" is absent
      ;; clean up any lingering comment face from before
      (let ((pos start))
        (while (< pos end)
          (let ((next (next-single-property-change pos 'font-lock-face nil end))
                (face (get-text-property pos 'font-lock-face)))
            (cond
             ;; case 1: it's exactly the comment face
             ((eq face 'font-lock-comment-face)
              (remove-text-properties pos next '(font-lock-face nil)))
             ;; case 2: it's a list containing the comment face
             ((and (listp face) (memq 'font-lock-comment-face face))
              (let ((new-face (delq 'font-lock-comment-face (copy-sequence face))))
                (put-text-property pos next 'font-lock-face
                                   (cond ((null new-face) nil)
                                         ((= (length new-face) 1) (car new-face))
                                         (t new-face))))))
            (setq pos next))))

      ;; finally, run standard highlighting to restore native syntax colors
      (funcall orig-fun lang start end))))

(advice-add 'org-src-font-lock-fontify-block :around #'efs/org-gray-out-tangle-no-blocks)
#+end_src
*** Remove ":tangle no" blocks from ":noweb-ref"
#+begin_src emacs-lisp :tangle org-tangle-advice.el
;; -*- lexical-binding: t; -*-
...
(use-package org-auto-tangle
  ...
  (org-auto-tangle-babel-safelist '(
     ...
     "/home/rafael/git/dotfiles/betterbird/policies.org"
     "/home/rafael/git/dotfiles/brave/json/extensions_full.org"
     "/home/rafael/git/dotfiles/brave/json/extensions_essential.org"
     "/home/rafael/git/dotfiles/cups/printers.org"
     "/home/rafael/git/dotfiles/i3-sway/config.org"
     "/home/rafael/git/dotfiles/mousam/dconf-settings.org"
     "/home/rafael/git/dotfiles/scripts/sway-dispatch.org"
     "/home/rafael/git/dotfiles/zen/policies.org"
     "/home/rafael/git/keebab/xremap/config.org"))
  ;; instruct org-auto-tangle background process to load the ":tangle no" advice
  (org-auto-tangle-with-files (list (expand-file-name "~/.config/emacs-efs/org-tangle-advice.el")))
  :config
  ;; load ":tangle no" advice into interactive session so it works with manual tangle
  (load (expand-file-name "~/.config/emacs-efs/org-tangle-advice.el") t)
#+end_src
*** Autoformat
**** Header
#+begin_src emacs-lisp
...
#+end_src
**** ini (dconf)
Post-tangle formatter for =.ini= files: removes backslashes, newlines, and surrounding whitespace from multiline blocks to generate the strict single-line format required by =dconf=. Example: https://github.com/ito-rafael/dotfiles/tree/master/mousam.
#+begin_src emacs-lisp
(when (and (buffer-file-name) (string-suffix-p ".ini" (buffer-file-name)))
  (message "Merging multiline strings in %s..." (buffer-file-name))
  (goto-char (point-min))
...
(let ((transfer-list `(
  ;; dotfiles
  (,(expand-file-name "~/git/dotfiles/dunst/dunstrc.org")         . (("dunstrc"              . "~/.config/dunst/dunstrc")))
  (,(expand-file-name "~/git/dotfiles/emacs/config.org")          . (("init.el"              . "~/.config/emacs-efs/init.el")
                                                                     ("org-tangle-advice.el" . "~/.config/emacs-efs/org-tangle-advice.el")))
  (,(expand-file-name "~/git/dotfiles/eww/osd.org")               . (("osd.sh"               . "~/.config/scripts/osd.sh")))
  (,(expand-file-name "~/git/dotfiles/eww/eww.org")               . (("eww.yuck"             . "~/.config/eww/eww.yuck")
                                                                     ("eww.scss"             . "~/.config/eww/eww.scss")))
  (,(expand-file-name "~/git/dotfiles/hypr/lock-screen.org")      . (("lock-screen.sh"       . "~/.config/hypr/lock-screen.sh")))
  (,(expand-file-name "~/git/dotfiles/hypr/hyprland.org")         . (("hyprland.lua"         . "~/.config/hypr/hyprland.lua")
                                                                     ("hyprpaper.conf"       . "~/.config/hypr/hyprpaper.conf")
                                                                     ("hyprland.conf"        . "~/.config/hypr/hyprland.conf")))
  (,(expand-file-name "~/git/dotfiles/i3-sway/config.org")        . (("_config_swayfx"       . "~/.config/sway/_config")
                                                                     ("_config_scroll"       . "~/.config/scroll/_config")
                                                                     ("_config_i3"           . "~/.config/i3/_config")))
  (,(expand-file-name "~/git/dotfiles/kanshi/config.org")         . (("config"               . "~/.config/kanshi/config")))
  (,(expand-file-name "~/git/dotfiles/kanshi/profile-setup.org")  . (("profile-setup.sh"     . "~/.config/scripts/kanshi-profile-setup.sh")))
  (,(expand-file-name "~/git/dotfiles/tlp/tlp.org")               . (("tlp.conf"             . "~/.config/tlp/tlp.conf")))
  ...
  (,(expand-file-name "~/git/dotfiles/zsh/zshrc.org")             . (("zshrc"                . "~/.config/zsh/.zshrc")))
...
***** Dunst
#+begin_src emacs-lisp
((string-match-p "dunst" target-dest)
 (call-process-shell-command "dunstctl reload" nil "*Messages*")
 (message "↻ Reloading Dunst..."))
#+end_src
***** Emacs
#+begin_src emacs-lisp
((string-match-p "emacs" target-dest)
 (call-process-shell-command "emacsclient -s efs -e '(efs/reload-emacs-config)'" nil 0)
 (message "↻ Reloadig Emacs config..."))
#+end_src
***** Hyprland
#+begin_src emacs-lisp
((string-match-p "hypr" target-dest)
 (call-process-shell-command "hyprctl reload" nil "*Messages*")
...
***** kanshi
#+begin_src emacs-lisp
((string-match-p "kanshi" target-dest)
 (call-process-shell-command "kanshictl reload" nil "*Messages*")
 (message "↻ Reloading kanshi..."))
#+end_src
***** Sway & Scroll
#+begin_src emacs-lisp
((string-match-p "/\\(sway\\|scroll\\)/" target-dest)
 (let* ((wm-dir (file-name-directory target-dest))
        (wm-name (if (string-match-p "sway" wm-dir) "sway" "scroll"))
        (script-path (expand-file-name "i3bang.rb" wm-dir))
        (config-source (expand-file-name "_config" wm-dir))
        (runtime-dir (format "/run/user/%s" (user-uid)))
        (sway-socket (car (file-expand-wildcards (expand-file-name "sway-ipc.*.sock" runtime-dir))))
        (scroll-socket (car (file-expand-wildcards (expand-file-name "scroll-ipc.*.sock" runtime-dir)))))

   ;; always run i3bang.rb to generate the config for this specific directory
   (let ((gen-cmd (format "ruby '%s' '%s'" script-path config-source)))
     (call-process-shell-command gen-cmd nil "*Messages*")
     (message "✔ Generated config for %s." wm-name))

   ;; only reload if the active window manager matches the current wm-dir
   (cond
     ;;------------------------
     ;; swaymsg reload
     ;;------------------------
     ;; if we are processing Sway and Sway is the active WM
     ((and (string= wm-name "sway") sway-socket)
      (let ((reload-cmd (format "export SWAY_SCALE=$(~/.config/scripts/sway-scale.sh get); swaymsg -s '%s' reload && sleep 1 && ~/.config/scripts/sway-scale.sh \"$SWAY_SCALE\"" sway-socket)))
        (call-process-shell-command reload-cmd nil "*Messages*")
        (message "↻ Reloaded active WM: Sway.")))

     ;;------------------------
     ;; scroll reload
     ;;------------------------
     ;; if we are processing Scroll and Scroll is the active WM
     ((and (string= wm-name "scroll") scroll-socket)
      (let ((reload-cmd (format "export SWAY_SCALE=$(~/.config/scripts/sway-scale.sh get); scrollmsg -s '%s' reload && sleep 1 && ~/.config/scripts/sway-scale.sh \"$SWAY_SCALE\"" scroll-socket)))
        (call-process-shell-command reload-cmd nil "*Messages*")
        (message "↻ Reloaded active WM: Scroll.")))
     ;;------------------------
     ;; neither compositor is running
     ;;------------------------
     ;; if we are processing a WM that is not currently running
     (t
       (message "Skipped reload: %s is not the currently active WM." wm-name)))))
#+end_src
***** xremap
#+begin_src emacs-lisp
...
((string-match-p "waybar" target-dest)
 (call-process-shell-command "systemctl --user reload waybar.service" nil "*Messages*")
...
(use-package dired
  ...
  (evil-collection-define-key 'normal 'dired-mode-map
    ...
    "o" 'dired-find-file
    "r" 'efs/dired-toggle-sort-reverse
  )
)
#+end_src
** Custom functions
...
#+begin_src emacs-lisp
;; reverse sorting
(defun efs/dired-toggle-sort-reverse ()
  "Toggle reverse sorting in the current Dired buffer."
  (interactive)
  (if (derived-mode-p 'dired-mode)
      (let* ((switches dired-actual-switches)
             (new-switches
              (if (string-match-p "\\`-[a-zA-Z]*r" switches)
                  ;; if 'r' is in the first block of short flags (like -aghor), strip the 'r'
                  (replace-regexp-in-string "\\(\\`-[a-zA-Z]*\\)r" "\\1" switches)
                ;; otherwise, append 'r' to that first block of short flags (like -aghotr)
                (replace-regexp-in-string "\\(\\`-[a-zA-Z]*\\)" "\\1r" switches))))
        ;; apply the new switches locally and refresh the buffer
...
+https://github.com/asok/peep-dired+ (deprecated)
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (efs/leader-keys
    "d"   '(:ignore t :wk "Dired")
    "d t" '(dired-create-empty-file  :wk "Switch buffer")
  )
#+end_src
** e --> eval / emoji
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (efs/leader-keys
    ...
    "f z" '((lambda () (interactive) (find-file "~/git/dotfiles/zsh/zshrc.org"))      :wk "Open zshrc dotfile")
  )
#+end_src
** g --> git
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (efs/leader-keys
    "m" '(:ignore t :wk "Miscellaneous")
    "m g" '(counsel-rg               :wk "ripgrep search")
    "m r" '(hydra-window-resize/body :wk "resize window")
    "m s" '(dired-do-symlink         :wk "create symlink")
    "m t" '(hydra-text-scale/body    :wk "scale text")
  )
#+end_src
** n --> navigate
#+begin_src emacs-lisp
  (efs/leader-keys
    ...
    "n n" '(evil-avy-goto-char-timer :wk "Avy char")
    "n e" '(evil-avy-goto-line       :wk "Avy line")
  )
#+end_src
  )
#+end_src
...
#+begin_src emacs-lisp
  (efs/leader-keys
    ...
    "r i" '(lambda () (interactive) (find-file "/ssh:ipf:~"))
...
#+begin_src emacs-lisp
  (efs/leader-keys
    ...
    "t c" '(pandoc-main-transient     :wk "Toggle pandoc menu")
    "t g" '(gptel-menu                :wk "Toggle gptel menu")
    "t k" '(keycast-mode              :wk "Toggle keycast")
    "t p" '(dired-preview-global-mode :wk "Toggle dired-preview")
  )
#+end_src
** w --> window
#+begin_src emacs-lisp
...
(use-package ivy
  ...
  ;; navigation on ivy-occur after counsel-rg (ripgrep)
  (with-eval-after-load 'evil
    (evil-define-key '(normal motion) ivy-occur-grep-mode-map
      (kbd "e") 'next-error
      (kbd "i") 'previous-error))
...
(use-package counsel
  ...
  :bind (:map
    counsel-ag-map
    ("TAB"   . ivy-occur)  ;; open ivy-occur after counsel-rg (ripgrep)
    ("<tab>" . ivy-occur)  ;; open ivy-occur after counsel-rg (ripgrep)
  )
  :config
  (counsel-mode 1)
...
#+end_src
** Resize window
Define transient keybindings for resizing windows.
#+begin_src emacs-lisp
(defhydra hydra-window-resize (:color red :hint nil :timeout 5)
  "Window Resize Mode"
  ("n" shrink-window-horizontally  "hor-")
  ("e" shrink-window               "ver-")
  ("i" enlarge-window              "ver+")
  ("o" enlarge-window-horizontally "hor+")
  ("C-n" (lambda () (interactive) (shrink-window-horizontally 10)) "hor--")
  ("C-e" (lambda () (interactive) (shrink-window 10)) "ver--")
  ("C-i" (lambda () (interactive) (enlarge-window 10)) "ver++")
  ("C-o" (lambda () (interactive) (enlarge-window-horizontally 10)) "hor++")
  ("s-a"      nil "exit" :color blue)
  ("<escape>" nil "exit" :color blue)
)
#+end_src
** EoS
#+begin_src emacs-lisp
...
(use-package magit
  ...
  :hook (
    (git-commit-post-finish . magit-status)       ;; return to magit-status after a commit
    (magit-status-mode . efs/magit-status-setup)  ;; set line number type to visual in magit-status
  )
  :custom
  ...
  :config
  ;; start git-commit-mode in insert mode
  ...
  (evil-collection-define-key '(normal visual) 'magit-status-mode-map
    ...
    "4" 'digit-argument
    ;; custom navigation
    (kbd "C-e") 'magit-section-forward             ;; next section
    (kbd "C-i") 'magit-section-backward            ;; previous section
    (kbd "<tab>") 'magit-section-toggle            ;; hide/show section
    (kbd "M-i") 'magit-section-show-level-2-all    ;; fold everything
    ;; major section navigation
    (kbd "M-n") 'efs/magit-previous-major-section  ;; previous major section
    (kbd "M-o") 'efs/magit-next-major-section      ;; next major section
  )
  ;;---------------------------
  ;; force "e" and "i" to use Magit's native logical line movements across all Magit buffers
  (evil-collection-define-key '(normal visual motion) 'magit-mode-map
    "e" 'magit-next-line
    "i" 'magit-previous-line
  )
  ;;---------------------------
  ;; return to magit-status smoothly after "C-c C-k" (cancel) or "C-c C-c" (finish)
  (defun efs/magit-restore-status-smooth (orig-fun &rest args)
    "Close commit without flashing dashboard, then restore layout.
    ...
    (let* ((repo (magit-toplevel))
      ...
      (let ((inhibit-redisplay t))
        ;; run the original command (finishes or cancels the commit buffer)
        (apply orig-fun args)
        ;; instantly swap to status buffer "in the dark"
        (when (and status-buf (buffer-live-p status-buf))
          (switch-to-buffer status-buf)
          ;; force single window (closes diff/staged changes window)
          (delete-other-windows)))
      ;; run full refresh to fix transient prefixes after a small delay
      (when repo
        (run-at-time 0.05 nil (lambda (dir) (magit-status dir)) repo))))

  ;; apply the advice to both finishing and cancelling a commit
  (advice-add 'with-editor-cancel :around #'efs/magit-restore-status-smooth)
  (advice-add 'with-editor-finish :around #'efs/magit-restore-status-smooth)
  ;;===========================
  ;; major section navigation
  ;;===========================
  ;; previous section (eg: Unstaged --> Untracked)
  (defun efs/magit-previous-major-section ()
    "Jump directly to the previous major section (Unstaged -> Untracked), skipping files."
    (interactive)
    (let ((sec (magit-current-section)))
      (when sec
        (while (and (oref sec parent)
                    (oref (oref sec parent) parent))
          (setq sec (oref sec parent)))
        (goto-char (oref sec start))
        (magit-section-backward-sibling))))
  ;;---------------------------
  ;; next section (eg: Untracked --> Unstaged)
  (defun efs/magit-next-major-section ()
    "Jump directly to the next major section (eg: Untracked -> Unstaged), skipping files."
    (interactive)
    (let ((sec (magit-current-section)))
      (when sec
        ;; Climb up the section tree using 'oref' until we reach a top-level section
        (while (and (oref sec parent)
                    (oref (oref sec parent) parent))
          (setq sec (oref sec parent)))
        ;; Move cursor to the major section header, then jump to its sibling
        (goto-char (oref sec start))
        (magit-section-forward-sibling))))
  ;;---------------------------
)
#+end_src
*** Maintain cursor after staging
Custom function to =magit-stage= ("s") while in =evil-visual-line= ("v").
#+begin_src emacs-lisp
(defun efs/magit-stage-keeping-line (orig-fun &rest args)
  "Advice to keep the cursor at the same line number after staging."
  (let ((current-line (line-number-at-pos)))
    ;; run the original magit-stage command
    (apply orig-fun args)
    ;; after the refresh, go back to the saved line
    (goto-char (point-min))
    (forward-line (1- current-line))))

;; apply the advice to magit-stage
(advice-add 'magit-stage :around #'efs/magit-stage-keeping-line)
#+end_src
*** Use visual line number in magit-status
Custom function to change the line number type to visual.
#+begin_src emacs-lisp
(defun efs/magit-status-setup ()
  "Custom settings for Magit status buffer."
  (visual-line-mode -1)                            ;; disable Emacs' visual remappings
  (setq-local truncate-lines nil)                  ;; allow lines to break/wrap visually
  (setq-local word-wrap t)                         ;; wrap nicely at word boundaries
  (setq-local line-move-visual nil)                ;; tell standard Emacs commands to use logical lines
...
(use-package forge
  :after magit
  :custom
  (forge-add-default-bindings nil) ;; silence the evil-collection warning
  :config
...
(use-package dashboard
  :custom
  (dashboard-image-extra-props '(:mask heuristic))  ;; enable image background transparency
  :init
...
(use-package windmove
  ...
  ;; helper function to replicate the logic from sway-dispatch.sh
  (defun efs/wm-focus-out-of-emacs (dir)
    "Focus window outside of Emacs based on DIR, respecting lan-mouse status."
    (let* ((session-type (getenv "XDG_SESSION_TYPE"))
           (lanmouse-file "/tmp/lanmouse-status.tmp")
           ;; safely read the lanmouse status file if it exists
           (lanmouse-status (when (file-exists-p lanmouse-file)
                              (with-temp-buffer
                                (insert-file-contents lanmouse-file)
                                (string-trim (buffer-string)))))
           ;; check if status is either connected or active
           (lanmouse-active (member lanmouse-status '("connected" "active")))

           ;; resolve the script path (fallback to ~/.config if XDG_CONFIG_HOME is unset)
           (config-home (or (getenv "XDG_CONFIG_HOME") (expand-file-name "~/.config")))
           (nav-script (expand-file-name "scripts/sway-window-nav.py" config-home))

           ;; construct the appropriate window manager command
           (wm-cmd (pcase session-type
                     ("x11"     (format "i3-msg focus %s" dir))
                     ("wayland" (if lanmouse-active
                                    (format "swaymsg exec %s %s" nav-script dir)
                                  (format "swaymsg focus %s" dir)))
                     (_         nil))))

      (when wm-cmd
        (shell-command (concat wm-cmd " > /dev/null 2>&1") nil nil))))

  ;; advice to intercept window boundary errors
  (defun efs/windmove-focus-out-advice (orig-fun dir &rest args)
    "Wrap `windmove-do-window-select` to trigger WM focus when hitting the frame edge."
    (condition-case err
        ;; try to run the native windmove logic first
        (apply orig-fun dir args)
      ;; catch the specific errors Emacs throws when it hits the edge of the frame
      (user-error
       (let ((err-str (error-message-string err)))
         (if (or (string-match-p "No window .* from selected window" err-str)
                 (string-match-p "Minibuffer is inactive" err-str))
             ;; if at the edge, pass focus to Sway / i3wm
             (efs/wm-focus-out-of-emacs dir)
           ;; otherwise, re-throw the error
           (signal (car err) (cdr err)))))))

  ;; apply the advice to the native windmove function
  (advice-add 'windmove-do-window-select :around #'efs/windmove-focus-out-advice))
#+end_src
* Buffer Menu
C-x C-b
!!! NOT WORKING !!!
...
(use-package buff-menu
  :ensure nil  ;; do not install (built-in)
  ;:no-require  ;; do not load package (just add the config to the eval-after-load hook)
  :after evil
  :bind (
       :map buffer-menu-mode-map
       ("C-e" . evil-next-line)        ;; down
       ("C-i" . evil-previous-line)))  ;; up
#+end_src
* Simple
Use SPACE for indentation instead of TAB.
#+begin_src emacs-lisp
(use-package simple
  :ensure nil  ;; do not install (already built-in)
  :no-require  ;; do not load package (just add the config to the eval-after-load hook)
  :custom
...
(use-package desktop
  ...
  :init
  ;; prevent desktop from saving image buffers to avoid PGTK daemon crashes
...
(use-package ace-window
  :custom
  ;; display text in minibuffer when ace-window is active
...
(use-package highlight-indent-guides
  ...
  :custom
  (highlight-indent-guides-method 'column)   ;; use a thick background line
  (highlight-indent-guides-responsive 'top)  ;; highlight active block
)
#+end_src
* emacs-everywhere
https://github.com/tecosaur/emacs-everywhere
Related:
  - GhostText (web-only): https://github.com/fregante/GhostText
#+begin_src emacs-lisp
(use-package emacs-everywhere
  :custom
  ;; set window title
  (emacs-everywhere-frame-name-format "Emacs Everywhere ∷ %s — %s")
  :config
  ;; force system config evaluation
  (emacs-everywhere--configure-system)

  ;; get built-in Sway config
  (let ((sway-config (alist-get '(wayland . sway) emacs-everywhere-system-configs nil nil #'equal)))

    (when sway-config
      ;; edit the copy command (%f is the temp file created by emacs-everywhere creates)
      (setq sway-config
        ;(plist-put wayland-config :copy-command '("sh" "-c" "wl-copy < %f")))
        (plist-put sway-config :copy-command '("ydotool" "key" "42:1" "186:1" "186:0" "42:0")))
      ;; edit the paste command to "Ctrl-F16"
      (setq sway-config
        ;(plist-put wayland-config :paste-command '("wtype" "-M" "Shift" "-P" "Insert" "-m" "Shift" "-p" "Insert")))
        (plist-put sway-config :paste-command '("ydotool" "key" "29:1" "186:1" "186:0" "29:0")))

      ;; assign this config to both 'sway and 'sway:wlroots
      (setf (alist-get '(wayland . sway) emacs-everywhere-system-configs nil nil #'equal) sway-config)
      (setf (alist-get '(wayland . sway:wlroots) emacs-everywhere-system-configs nil nil #'equal) sway-config)

      ;; wipe the currently cached system config and force a re-detect
      (setq emacs-everywhere-system-config nil)
      (emacs-everywhere--configure-system)
    )
  )
)
...
#+end_src
* rainbow-mode
Give colors on hex codes (eg: #ff0000).
...
(use-package rainbow-mode
  ...
  :config
  ;; define function to handle only RGBA colors
  (defun my-rainbow-colorize-rgba ()
    "Colorize 8-digit and 4-digit hex colors by ignoring the alpha channel."
    (let ((color (match-string-no-properties 0)))
      (cond
       ;; if 8-digit (#RRGGBBAA), colorize using the first 7 chars (#RRGGBB)
       ((= (length color) 9) (rainbow-colorize-match (substring color 0 7)))
       ;; if 4-digit (#RGBA), colorize using the first 4 chars (#RGB)
       ((= (length color) 5) (rainbow-colorize-match (substring color 0 4))))))

  ;; add a new font-lock rule specifically for length-5 and length-9 hex codes
  (add-to-list 'rainbow-hexadecimal-colors-font-lock-keywords
               '("#[0-9a-fA-F]\\{4\\}\\([0-9a-fA-F]\\{4\\}\\)?\\>"
                 (0 (my-rainbow-colorize-rgba))) t)
)
#+end_src
* Avy
...
(use-package avy
  :custom
  ;; enable avy search in all windows
  (avy-all-windows t)
  ;; dim background text during a jump
  (avy-background t)

  ;; hint characters
  ;;avy-keys '(?a ?s ?d ?f ?g ?h ?j ?k ?l) ;; original qwerty
  (avy-keys '(?a ?r ?s ?t ?n ?e ?i ?o)) ;; Colemak-DH
...
(use-package gptel
  ...
  :config
  ;; define Google Gemini as your LLM backend
  (setq gptel-backend
    (gptel-make-gemini "Gemini"
      ;; read API key with =pass= (instead of exposing it in the config)
      :key (lambda ()
