...
((string-match-p "sway" target-dest)
 (let* ((sway-dir (file-name-directory target-dest))
        ...
        (cmd (format "export SWAY_SCALE=$(~/.config/scripts/sway-scale.sh get); export SWAYSOCK=$(ls /run/user/$(id -u)/sway-ipc.*.sock | head -n 1); ruby '%s' '%s' && swaymsg reload && sleep 1 && ~/.config/scripts/sway-scale.sh $SWAY_SCALE"
                     script-path
                     config-source)))
