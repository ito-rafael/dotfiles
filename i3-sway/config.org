...
default_border pixel 5                  # set window border
floating_modifier $mod                  # use Mouse+$mod to drag floating windows
workspace_auto_back_and_forth yes       # enable previous workspace switching
gaps inner 10                           # gaps size
focus_on_window_activation urgent       # give window alerts instead of stealing focus
#+end_src
** Font & color
...
client.focused          #8cB8D9 #285577 #ffffff #285587   #2e7ea4
client.focused_inactive #333333 #5f676a #ffffff #484e50   #2f373a
...
input type:touchpad {
    ...
    drag_lock disabled      # stop dragging after lifting from touchpad
}
#+end_src
*** Keyboard
**** Generic
...
input type:keyboard {
    ...
    xkb_layout "us"
    xkb_variant "intl"
    xkb_options "modifiers:modifiers,fnkeys:fnkeys"
}
#+end_src
    xkb_options "modifiers:modifiers,fnkeys:fnkeys"
**** Disable CapsLock
...
input "1:1:AT_Translated_Set_2_keyboard" {
    xkb_layout "us"
    xkb_variant "intl"
    xkb_options "modifiers:modifiers,fnkeys:fnkeys,capslockled:decouple"
}
input "1:1:kanata" {
    xkb_layout "us"
    xkb_variant "intl"
...
#+end_src
* SwayFX
#+begin_src conf :noweb-ref swayfx
animation_duration_ms 250  # enable smooth window animations
corner_radius 20
...
bindsym XF86AudioRaiseVolume exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+ && ~/.config/scripts/osd.sh volume
bindsym XF86AudioLowerVolume exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%- && ~/.config/scripts/osd.sh volume
#+end_src
**** Fine
#+begin_src conf :noweb-ref volume
bindsym shift+XF86AudioRaiseVolume exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SINK@ 1%- && ~/.config/scripts/osd.sh volume
bindsym shift+XF86AudioLowerVolume exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SINK@ 1%- && ~/.config/scripts/osd.sh volume
#+end_src
**** Toggle
#+begin_src conf :noweb-ref volume
bindsym XF86AudioMute exec --no-startup-id wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle && ~/.config/scripts/osd.sh volume
#+end_src
** Microphone control
...
bindsym F14      exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 5%- && ~/.config/scripts/osd.sh mic
bindsym ctrl+F14 exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 5%+ && ~/.config/scripts/osd.sh mic
#+end_src
**** Fine
#+begin_src conf :noweb-ref microphone :tangle no
bindsym shift+F14      exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 1%- && ~/.config/scripts/osd.sh mic
bindsym ctrl+shift+F14 exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 1%+ && ~/.config/scripts/osd.sh mic
#+end_src
**** Toggle
#+begin_src conf :noweb-ref microphone
bindsym XF86AudioMicMute exec --no-startup-id wpctl set-volume @DEFAULT_AUDIO_SOURCE@ toggle && ~/.config/scripts/osd.sh mic
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
Deprecated: all keybindings were moved to =xremap=.
*** Current window
...
bindsym $mod+Left  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus left
bindsym $mod+Down  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus down
bindsym $mod+Up    exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus up
bindsym $mod+Right exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus right
#+end_src
** Move window
...
bindsym $mod+Shift+Left  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move left
bindsym $mod+Shift+Down  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move down
bindsym $mod+Shift+Up    exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move up
bindsym $mod+Shift+Right exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move right
# for Emacs
bindsym Ctrl+$mod+Shift+Left  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move left
bindsym Ctrl+$mod+Shift+Down  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move down
bindsym Ctrl+$mod+Shift+Up    exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move up
bindsym Ctrl+$mod+Shift+Right exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh move right
#+end_src
** Other focus
...
#bindsym mod1+$mod+f exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh fullscreen
#bindsym Shift+mod1+$mod+f fullscreen toggle
bindsym $mod+f exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh fullscreen  # SwayFX bug
bindsym Shift+$mod+f fullscreen toggle                                    # SwayFX bug
#+end_src
*** Tiling/Floating
...
#bindsym mod1+$mod+q exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh kill
#bindsym shift+mod1+$mod+q exec $SCRIPT_PATH/kill-window.sh
bindsym $mod+q exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh kill  # SwayFX bug
bindsym shift+$mod+q exec $SCRIPT_PATH/kill-window.sh               # SwayFX bug
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
#+begin_src conf :noweb-ref sway-restart
# exit from Sway + kill systemd graphical session target
bindsym mod1+$mod+x exec swaynag -t warning -m 'You pressed the exit shortcut. Do you really want to exit sway? This will end your Wayland session.' -B 'Yes, exit sway' 'systemctl --user stop graphical-session.target; swaymsg exit'
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
bindsym shift+mod3+Left  exec "$XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus_output left"
bindsym Ctrl+$mod+Left   exec "$XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus_output left"
bindsym shift+mod3+Right exec "$XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus_output right"
bindsym Ctrl+$mod+Right  exec "$XDG_CONFIG_HOME/scripts/sway-dispatch.sh focus_output right"
#+end_src
...
bindsym Shift+F16 nop
bindsym Ctrl+F16 nop
bindsym $alt+F16 nop
bindsym --release $alt+F16  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh cut
bindsym --release Shift+F16 exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh copy
bindsym --release Ctrl+F16  exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh paste
#+end_src
** Applications
#+begin_src conf :noweb-ref keybindings
...
bindsym mod1+$mod+g exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh new_terminal
#bindsym mod1+$mod+g exec kitty
#bindsym $mod+g exec i3-sensible-terminal
#bindsym mod1+$mod+g exec i3-sensible-terminal
#bindsym ctrl+$mod+g exec i3-sensible-terminal
#+end_src
** Brightness
*** Screen brightness
**** xbacklight
...
**** brightnessctl
#+begin_src conf :noweb-ref keybindings
bindsym XF86MonBrightnessUp         exec brightnessctl set 5%+ && ~/.config/scripts/osd.sh brightness
bindsym XF86MonBrightnessDown       exec brightnessctl set 5%- && ~/.config/scripts/osd.sh brightness
bindsym shift+XF86MonBrightnessUp   exec brightnessctl set 1%+ && ~/.config/scripts/osd.sh brightness
bindsym shift+XF86MonBrightnessDown exec brightnessctl set 1%- && ~/.config/scripts/osd.sh brightness
#+end_src
*** Keyboard backlit
#+begin_src conf :noweb-ref keybindings
bindsym XF86KbdBrightnessDown   exec "$SCRIPT_PATH/keeb-backlit.sh dec && ~/.config/scripts/osd.sh backlit"
bindsym XF86KbdBrightnessUp     exec "$SCRIPT_PATH/keeb-backlit.sh inc && ~/.config/scripts/osd.sh backlit"
#bindsym XF86KbdBrightnessToggle exec "$SCRIPT_PATH/keeb-backlit.sh toggle && ~/.config/scripts/osd.sh backlit"
bindsym XF86KbdLightOnOff exec "$SCRIPT_PATH/keeb-backlit.sh toggle && ~/.config/scripts/osd.sh backlit"
#+end_src
** Rofi
...
bindsym $alt+ctrl+m exec "$SCRIPT_PATH/wlprop.sh show"
#+end_src
** Translations
#+begin_src conf :noweb-ref keybindings :tangle no
#bindsym $mod+t exec dbus-send --type=method_call --dest=io.crow_translate.CrowTranslate /io/crow_translate/CrowTranslate/MainWindow io.crow_translate.CrowTranslate.MainWindow.translateSelection
#bindsym $mod+Shift+t exec dbus-send --type=method_call --dest=io.crow_translate.CrowTranslate /io/crow_translate/CrowTranslate/MainWindow io.crow_translate.CrowTranslate.MainWindow.speakSelection
#bindsym $alt+ctrl+j exec dbus-send --type=method_call --dest=io.crow_translate.CrowTranslate /io/crow_translate/CrowTranslate/MainWindow io.crow_translate.CrowTranslate.MainWindow.translateSelection
#bindsym $alt+ctrl+h exec dbus-send --type=method_call --dest=io.crow_translate.CrowTranslate /io/crow_translate/CrowTranslate/MainWindow io.crow_translate.CrowTranslate.MainWindow.speakSelection
#+end_src
** Others
*** lan-mouse switch
Change host focus with switch KVM.
#+begin_src conf :noweb-ref keybindings
bindsym $mod+m nop
bindsym --release $mod+m exec "$XDG_CONFIG_HOME/scripts/lanmouse-switch.sh toggle"
#+end_src
*** Debug
Debug/test purposes: replace kitty with a custom command or script.
#+begin_src conf :noweb-ref keybindings
bindsym --release $alt+ctrl+t exec "$XDG_CONFIG_HOME/scripts/sway-test.sh"
#+end_src
*** Start/pause pomodoro timer
#+begin_src conf :noweb-ref keybindings
bindsym $mod+j exec tomat toggle
#+end_src
*** Theme toggle
Change the system theme {dark,light} via gsettings.
#+begin_src conf :noweb-ref keybindings
bindsym Shift+$mod+t exec "$XDG_CONFIG_HOME/scripts/theme-toggle.sh"
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
#+begin_src conf :tangle no
bindsym mod3+button4 --whole-window exec "pactl set-sink-volume @DEFAULT_SINK@ +5% && pkill -RTMIN+10 i3blocks"
bindsym mod3+button5 --whole-window exec "pactl set-sink-volume @DEFAULT_SINK@ -5% && pkill -RTMIN+10 i3blocks"
#+end_src
...
set $locker $XDG_CONFIG_HOME/hypr/lock-screen.sh
# exit from Sway + kill systemd graphical session target
set $messenger "systemctl --user stop graphical-session.target && swaymsg exit"
#+end_src
**** Mode
Obs: the -i argument for systemctl poweroff causes a shutdown even if other users are logged-in (this requires polkit), or when logind (wrongly) assumes so.
#+begin_src conf :noweb-ref sway-mode-system
...
mode "resize" {
    ...
    #=============================
    # colemak home row
    bindsym ctrl+n resize shrink width  20 px or 20 ppt
    bindsym ctrl+e resize shrink height 20 px or 20 ppt
    bindsym ctrl+i resize grow   height 20 px or 20 ppt
    bindsym ctrl+o resize grow   width  20 px or 20 ppt
    #-----------------------------
    # same bindings, but for the arrow keys
    bindsym ctrl+Left  resize shrink width  20 px or 20 ppt
    bindsym ctrl+Down  resize shrink height 20 px or 20 ppt
    bindsym ctrl+Up    resize grow   height 20 px or 20 ppt
...
*** Keybinding
#+begin_src conf :noweb-ref mode-resize
bindsym $mod+a exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh resize
bindsym shift+$mod+a mode "resize"
#+end_src
** Number
*** Description
...
*** Acute/Tilde
**** F13
#+begin_src conf :noweb-ref mode-f13
set $mode_acute acute
mode "$mode_acute" {
    # apostrophe
    bindsym F13     exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'", mode "default"
    bindsym space   exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'", mode "default"
    # cedilla
    bindsym c       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "ç", mode "default"
    bindsym Shift+c exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "Ç", mode "default"
    # vowel lowercase
    bindsym a       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "á", mode "default"
    bindsym e       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "é", mode "default"
    bindsym i       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "í", mode "default"
    bindsym o       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "ó", mode "default"
    bindsym u       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "ú", mode "default"
    # vowel uppercase
    bindsym Shift+a exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "Á", mode "default"
    bindsym Shift+e exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "É", mode "default"
    bindsym Shift+i exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "Í", mode "default"
    bindsym Shift+o exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "Ó", mode "default"
    bindsym Shift+u exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "Ú", mode "default"
    # misc lowercase
    bindsym l       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'l", mode "default"
    bindsym n       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'n", mode "default"
    bindsym m       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'m", mode "default"
    bindsym r       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'r", mode "default"
    bindsym s       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'s", mode "default"
    bindsym t       exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'t", mode "default"
    # misc uppercase
    bindsym Shift+l exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'L", mode "default"
    bindsym Shift+n exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'N", mode "default"
    bindsym Shift+m exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'M", mode "default"
    bindsym Shift+r exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'R", mode "default"
    bindsym Shift+s exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'S", mode "default"
    bindsym Shift+t exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'T", mode "default"
    # escape
    bindsym Escape mode "default"
    bindsym Return mode "default"
    #-----------------------------
...
**** S-F13
#+begin_src conf :noweb-ref mode-f13
set $mode_tilde tilde
mode "$mode_tilde" {
    # double quotes
    bindsym F13   exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '"', mode "default"
    bindsym space exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '"', mode "default"
    # tilde lowercase
    bindsym a exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'ã', mode "default"
    bindsym o exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'õ', mode "default"
    bindsym n exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'ñ', mode "default"
    # tilde uppercase
    bindsym Shift+a exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'Ã', mode "default"
    bindsym Shift+o exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'Õ', mode "default"
    bindsym Shift+n exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type 'Ñ', mode "default"
    # tilde misc
    bindsym Shift+F13 exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~',   mode "default"
    bindsym slash     exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/',  mode "default"
    bindsym period    exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.', mode "default"
    # tilde paths
    bindsym h exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/',                   mode "default"
    bindsym g exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/git/',               mode "default"
    bindsym k exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/git/keebab/',        mode "default"
    bindsym d exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/git/dotfiles/',      mode "default"
    bindsym l exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.local/',            mode "default"
    bindsym c exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/',           mode "default"
    bindsym e exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/emacs-efs/', mode "default"
    bindsym s exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/scripts/',   mode "default"
    bindsym v exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/nvim/',      mode "default"
    bindsym x exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/xremap/',    mode "default"
    bindsym w exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/waybar/',    mode "default"
    bindsym z exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/zsh/',       mode "default"
    bindsym i exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type '~/.config/sway/',      mode "default"
    # escape
    bindsym Escape mode "default"
    bindsym Return mode "default"
    #-----------------------------
...
**** Keybinding
#+begin_src conf :noweb-ref mode-f13
bindsym F13 mode "$mode_acute"
bindsym Shift+F13 mode "$mode_tilde"
#+end_src
*** System control
**** Speaker
***** Mode
****** Attempt 1
#+begin_src conf :noweb-ref mode-control2 :tangle no
set $mode_speaker speaker
mode "$mode_speaker" {
    bindsym space   exec $XDG_CONFIG_HOME/scripts/sway-dispatch.sh type "'"
    bindsym Escape mode "default"
    bindsym --release F24 mode "default"
    bindsym --release F20 mode "default"
}
bindsym F19 mode "$mode_speaker"
bindsym F20 mode "$mode_speaker"
bindsym F21 mode "$mode_speaker"
bindsym F22 mode "$mode_speaker"
bindsym F23 mode "$mode_speaker"
bindsym F24 mode "$mode_speaker"
#bindsym --whole-window button4 exec amixer -q sset Master 5%+
#bindsym --whole-window button5 exec amixer -q sset Master 5%-
#bindsym --whole-window button1 exec "$XDG_CONFIG_HOME/scripts/volume-control.sh down 25"
#bindsym --whole-window button2 exec "$XDG_CONFIG_HOME/scripts/volume-control.sh up 25"
#bindsym --whole-window button3 exec amixer -q sset Master 50%
#+end_src
****** Attempt 2
#+begin_src conf :noweb-ref mode-control :tangle no
set $mode_speaker speaker
mode "$mode_speaker" {
    # toggle
    bindsym k         exec amixer -q sset Master toggle
    # levels
    bindsym Escape    exec amixer -q sset Master 0%
    bindsym space     exec amixer -q sset Master 25%
    bindsym Tab       exec amixer -q sset Master 50%
    bindsym Return    exec amixer -q sset Master 75%
    bindsym BackSpace exec amixer -q sset Master 100%
    # coarse
    bindsym e         exec amixer -q sset Master 5%-
    bindsym i         exec amixer -q sset Master 5%+
    # fine
    bindsym Shift+e   exec amixer -q sset Master 1%-
    bindsym Shift+i   exec amixer -q sset Master 1%+
    bindsym comma     exec amixer -q sset Master 1%-
    bindsym period    exec amixer -q sset Master 1%+
    # escape
    # note the "--release" flag to simulate a modifier
    bindsym --release F15 mode "default"
    #-----------------------------
    # inherent default keybinds (i3bang)
    #!@default_keybindings
}
...
*** Mouse
**** Side-bottom (F21)
#+begin_src conf :noweb-ref mode-mouse :tangle no
set $mode_mouse_side_bottom mouse-volume
mode "$mode_mouse_side_bottom" {
    # volume control
    bindsym --whole-window button4 exec amixer -q sset Master 5%+
    bindsym --whole-window button5 exec amixer -q sset Master 5%-
    bindsym --whole-window button1 exec "$XDG_CONFIG_HOME/scripts/volume-control.sh down 25"
    bindsym --whole-window button2 exec "$XDG_CONFIG_HOME/scripts/volume-control.sh up 25"
    bindsym --whole-window button3 exec amixer -q sset Master 50%
    bindsym Return exec amixer -q sset Master 98%
    # gestures
    bindsym F17     exec amixer -q sset Master 11%+
    bindsym F18     exec amixer -q sset Master 22%-
    bindsym Shift+n exec amixer -q sset Master 33%+
    bindsym Shift+o exec amixer -q sset Master 44%-
    # escape
    # note the "--release" flag to simulate a modifier
    bindsym --release F21 mode "default"
    #-----------------------------
    # inherent default keybinds (i3bang)
    #!@default_keybindings
}
...
**** Side-top (F22)
#+begin_src conf :noweb-ref mode-mouse :tangle no
set $mode_mouse_side_top mouse-zoom
mode "$mode_mouse_side_top" {
    # zoom control
    bindsym --whole-window button4 Ctrl+minus
    bindsym --whole-window button5 Shift+Ctrl+equal
    bindsym --whole-window button1 workspace prev_on_output
    bindsym --whole-window button2 workspace next_on_output
    bindsym --whole-window button3 exec amixer -q sset Master 50%
    bindsym Return exec amixer -q sset Master 76%
    # gestures
    bindsym F17     exec amixer -q sset Master 55%+
    bindsym F18     exec amixer -q sset Master 66%-
    bindsym Shift+n exec amixer -q sset Master 77%+
    bindsym Shift+o exec amixer -q sset Master 88%-
    # escape
    # note the "--release" flag to simulate a modifier
    bindsym --release F22 mode "default"
    #-----------------------------
    # inherent default keybinds (i3bang)
    #!@default_keybindings
}
#+end_src
**** Keybinding
#+begin_src conf :noweb-ref mode-mouse
#bindsym F21 mode "$mode_mouse_side_bottom"
#bindsym F22 mode "$mode_mouse_side_top"
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
#+end_src
*** Window properties
Display output of {xprop,wlprop}.sh scripts.
#+begin_src conf :noweb-ref window-property
for_window [$prop="emacs-window-prop"] floating enable
#+end_src
*** Rofi
#+begin_src conf :noweb-ref window-property
...
for_window [$prop="showmethekey-gtk"] {
    floating enable
    sticky enable
    resize set width 1720 height 75
    # x=0; y=(width - 200)
    move position 100 1000
}
for_window [$prop="one.alynx.showmethekey"] {
    floating enable
    sticky enable
    resize set width 1720 height 75
    # x=0; y=(width - 200)
...
#+end_src
*** lan-mouse
(deprecated) Proxy window used to lose main window focus when switching from sender to receiving end with lan-mouse (software switch KVM).
  +--> using xremap modes instead
#+begin_src conf :noweb-ref sway-lanmouse
#for_window [app_id="edge_bottom" title="pixel.png"] {
#    floating enable
#    sticky enable
#    resize set 100 ppt 1 px
#    move position 0 1079
#    opacity 0.0
#    border none
#}
#exec swayimg ~/.config/swayimg/pixel.png --class="edge_bottom"
#+end_src
*** Emacs
#+begin_src conf :noweb-ref sway-gaps :tangle no
for_window [$prop="emacs-30-2"] border pixel 20
#+end_src
*** emacs-everywhere
#+begin_src conf :noweb-ref sway-emacs
for_window [app_id="emacs" title="^(emacs-everywhere|Emacs Everywhere|Emacs Everywhere ∷)"] floating enable, resize set 700 400, move position center
for_window [class="Emacs" title="^(emacs-everywhere|Emacs Everywhere|Emacs Everywhere ∷)"] floating enable, resize set 700 400, move position center
#+end_src
*** ZSA firmware update
Floating window for updating the firmware of ZSA keyboards (eg: Moonlander, Planck EZ).
#+begin_src conf :noweb-ref zsa-update
for_window [$prop="zsa_update"] {
    floating enable
    sticky enable
    resize set width 500 height 200
    move position center
}
#+end_src
*** Informant
Arch Linux news.
#+begin_src conf :noweb-ref informant-news
for_window [$prop="informant-news"] {
    floating enable
    sticky enable
    resize set width 1000 height 1000
    move position center
}
#+end_src
*** ansible-provision
#+begin_src conf :noweb-ref ansible-provision
for_window [$prop="local.ansible-provision"] {
    floating enable
    move position center
}
#+end_src
*** xeyes
**** Sway
#+begin_src conf :noweb-ref xeyes
...
for_window [$prop="^dropdown_ansible$"] floating enable
for_window [$prop="^dropdown_ansible$"] move position center
for_window [$prop="^dropdown_ansible$"] move scratchpad
for_window [$prop="^dropdown_ansible$"] border pixel 5
bindsym $alt+$mod+k exec "$SCRIPT_PATH/show-or-launch.sh dropdown_ansible 0.75 0.75"
#+end_src
** AUR
#+begin_src conf :noweb-ref scratchpads
for_window [$prop="^dropdown_aur$"] floating enable
for_window [$prop="^dropdown_aur$"] move position center
for_window [$prop="^dropdown_aur$"] move scratchpad
for_window [$prop="^dropdown_aur$"] border pixel 5
bindsym $alt+$mod+v exec "$SCRIPT_PATH/show-or-launch.sh dropdown_aur 0.75 0.75"
#+end_src
** Betterbird
The obligation to move the window to a scratchpad + set a visible opacity is delegated to the =show-or-launch.sh= script.
#+begin_src conf :noweb-ref scratchpads
for_window [$prop="^eu.betterbird.Betterbird$"] floating enable
for_window [$prop="^eu.betterbird.Betterbird$"] move position center
for_window [$prop="^eu.betterbird.Betterbird$"] border pixel 5
# wait initial rendering before moving to scratchpad
for_window [$prop="^eu.betterbird.Betterbird$"] move scratchpad
exec --no-startup-id bash -c 'betterbird & while ! swaymsg -t get_tree | grep -q "eu.betterbird.Betterbird"; do sleep 0.5; done; sleep 0.5; swaymsg "[app_id=\"^eu.betterbird.Betterbird$\"] move scratchpad"'
bindsym $alt+$mod+r exec "$SCRIPT_PATH/show-or-launch.sh eu.betterbird.Betterbird 0.99 0.965"
#+end_src
** Mousam
Weather at a glance.
#+begin_src conf :noweb-ref scratchpads
for_window [$prop="^io.github.amit9838.mousam$"] floating enable
for_window [$prop="^io.github.amit9838.mousam$"] move position center
for_window [$prop="^io.github.amit9838.mousam$"] move scratchpad
for_window [$prop="^io.github.amit9838.mousam$"] border pixel 5
bindsym $alt+$mod+d exec "$SCRIPT_PATH/show-or-launch.sh io.github.amit9838.mousam"
#+end_src
** WhatsApp Web (on Brave Beta)
*** i3
#+begin_src conf :noweb-ref i3-whatsapp-web
...
#bindsym $mod+t exec "$SCRIPT_PATH/show-or-launch.sh web.whatsapp.com 0.6 0.9"
#+end_src
*** Sway
#+begin_src conf :noweb-ref sway-whatsapp-web
...
#    border pixel 5
#}
##exec distrobox-enter -n brave-origin -- brave-origin --skip-origin-startup-dialog --app=https://web.whatsapp.com
#exec "$SCRIPT_PATH/pwa-launcher.sh whatsapp"
#bindsym $mod+t exec "$SCRIPT_PATH/show-or-launch.sh brave-web.whatsapp.com__-Default 0.6 0.9"
#+end_src
** WhatsApp (WasIstLos)
*** i3
#+begin_src conf :noweb-ref i3-whatsapp-wasistlos
for_window [$prop="^wasistlos$"] floating enable
for_window [$prop="^wasistlos$"] move position center
for_window [$prop="^wasistlos$"] move scratchpad
for_window [$prop="^wasistlos$"] border pixel 5
exec --no-startup-id wasistlos
bindsym $mod+t exec "$SCRIPT_PATH/show-or-launch.sh wasistlos 0.6 0.9"
#+end_src
*** Sway
#+begin_src conf :noweb-ref sway-whatsapp-wasistlos
for_window [$prop="^wasistlos$"] {
    floating enable
    move position center
    move scratchpad
    border pixel 5
...
bindsym $mod+s exec "$SCRIPT_PATH/show-or-launch.sh music.youtube.com 0.9 0.9"
#+end_src
*** Sway
...
#exec distrobox-enter -n brave-origin -- brave-origin --skip-origin-startup-dialog --app=https://music.youtube.com
exec "$SCRIPT_PATH/pwa-launcher.sh ytmusic"
bindsym $mod+s exec "$SCRIPT_PATH/show-or-launch.sh brave-music.youtube.com__-Default 0.9 0.9"
#+end_src
** Calendar (on Brave Beta)
*** i3
#+begin_src conf :noweb-ref i3-calendar
for_window [$prop="^Brave-browser-beta$" instance="^calendar.google.com$"] floating enable
for_window [$prop="^Brave-browser-beta$" instance="^calendar.google.com$"] move position center
for_window [$prop="^Brave-browser-beta$" instance="^calendar.google.com$"] move scratchpad
for_window [$prop="^Brave-browser-beta$" instance="^calendar.google.com$"] border pixel 5
exec --no-startup-id brave-beta --app=https://calendar.google.com
bindsym $mod+r exec "$SCRIPT_PATH/show-or-launch.sh calendar.google.com 0.6 0.9"
#+end_src
*** Sway
#+begin_src conf :noweb-ref sway-calendar
for_window [$prop="^brave-calendar.google.com__-Default$"] {
    floating enable
    move position center
    move scratchpad
    border pixel 5
}
#exec distrobox-enter -n brave-origin -- brave-origin --skip-origin-startup-dialog --app=https://calendar.google.com
exec "$SCRIPT_PATH/pwa-launcher.sh calendar"
bindsym $mod+r exec "$SCRIPT_PATH/show-or-launch.sh brave-calendar.google.com__-Default 0.6 0.9"
#+end_src
** Terminal
#+begin_src conf :noweb-ref scratchpads
...
bindsym $mod+n exec "$SCRIPT_PATH/show-or-launch.sh dropdown_terminal 0.75 0.75"
...
** pacman
#+begin_src conf :noweb-ref scratchpads
for_window [$prop="^dropdown_pacman$"] floating enable
for_window [$prop="^dropdown_pacman$"] move position center
for_window [$prop="^dropdown_pacman$"] move scratchpad
for_window [$prop="^dropdown_pacman$"] border pixel 5
bindsym $mod+v exec "$SCRIPT_PATH/show-or-launch.sh dropdown_pacman 0.75 0.75"
#+end_src
** Python
#+begin_src conf :noweb-ref scratchpads
...
bindsym $mod+e exec "$SCRIPT_PATH/show-or-launch.sh dropdown_python 0.6 0.6"
#+end_src
** scrcpy
#+begin_src conf :noweb-ref scratchpads
...
*** Phone
#+begin_src conf :noweb-ref scratchpads
bindsym $mod+k exec "$SCRIPT_PATH/adb-mdns-scrcpy.sh phone"
#+end_src
*** Watch
#+begin_src conf :noweb-ref scratchpads
#bindsym $mod+v exec "$SCRIPT_PATH/adb-mdns-scrcpy.sh watch"
#+end_src
** ZSA Keymapp
*** i3
...
bindsym $mod+slash exec "$SCRIPT_PATH/show-or-launch.sh Keymapp 0.75 0.75"
#+end_src
*** Sway
...
** LLM (on Brave Beta)
*** i3
#+begin_src conf :noweb-ref i3-llm
for_window [$prop="^Brave-browser-beta$" instance="^chatgpt.com$"] floating enable
for_window [$prop="^Brave-browser-beta$" instance="^chatgpt.com$"] move position center
for_window [$prop="^Brave-browser-beta$" instance="^chatgpt.com$"] move scratchpad
for_window [$prop="^Brave-browser-beta$" instance="^chatgpt.com$"] border pixel 5
exec --no-startup-id brave-beta --app=https://chatgpt.com
bindsym $alt+$mod+t exec "$SCRIPT_PATH/show-or-launch.sh chatgpt.com 0.6 0.9"
#+end_src
*** Sway
#+begin_src conf :noweb-ref sway-llm
for_window [$prop="^brave-chatgpt.com__-Default$"] {
    floating enable
    move position center
    move scratchpad
    border pixel 5
}
#exec distrobox-enter -n brave-origin -- brave-origin --skip-origin-startup-dialog --app=https://chatgpt.com
exec "$SCRIPT_PATH/pwa-launcher.sh chatgpt"
bindsym $alt+$mod+t exec "$SCRIPT_PATH/show-or-launch.sh brave-chatgpt.com__-Default 0.6 0.9"
#+end_src
** Translate (on Brave Beta)
*** i3
#+begin_src conf :noweb-ref i3-translate
for_window [$prop="^Brave-browser-beta$" instance="^translate.com$"] floating enable
for_window [$prop="^Brave-browser-beta$" instance="^translate.com$"] move position center
for_window [$prop="^Brave-browser-beta$" instance="^translate.com$"] move scratchpad
for_window [$prop="^Brave-browser-beta$" instance="^translate.com$"] border pixel 5
exec --no-startup-id brave-beta --app=https://translate.google.com
bindsym $alt+$mod+s exec "$SCRIPT_PATH/show-or-launch.sh calendar.google.com 0.6 0.9"
#+end_src
*** Sway
#+begin_src conf :noweb-ref sway-translate
for_window [$prop="^brave-translate.google.com__-Default$"] {
    floating enable
    move position center
    move scratchpad
    border pixel 5
}
#exec distrobox-enter -n brave-origin -- brave-origin --skip-origin-startup-dialog --app=https://translate.google.com
exec "$SCRIPT_PATH/pwa-launcher.sh translate"
bindsym $alt+$mod+s exec "$SCRIPT_PATH/show-or-launch.sh brave-translate.google.com__-Default 0.6 0.9"
#+end_src
** Temporary scratchpads
*** 1: h
#+begin_src conf :noweb-ref scratchpads
exec --no-startup-id "rm /tmp/scratchpad_pid_1.tmp"
bindsym $mod+h       exec "$SCRIPT_PATH/scratchpad-temp.sh 1 create-show"
bindsym shift+$mod+h exec "$SCRIPT_PATH/scratchpad-temp.sh 1 detach"
bindsym ctrl+$mod+h  exec "$SCRIPT_PATH/scratchpad-temp.sh 1 destroy"
#+end_src
*** 2: comma
#+begin_src conf :noweb-ref scratchpads
exec --no-startup-id "rm /tmp/scratchpad_pid_2.tmp"
bindsym $mod+comma       exec "$SCRIPT_PATH/scratchpad-temp.sh 2 create-show"
bindsym shift+$mod+comma exec "$SCRIPT_PATH/scratchpad-temp.sh 2 detach"
bindsym ctrl+$mod+comma  exec "$SCRIPT_PATH/scratchpad-temp.sh 2 destroy"
#+end_src
*** 3: dot
#+begin_src conf :noweb-ref scratchpads
exec --no-startup-id "rm /tmp/scratchpad_pid_3.tmp"
bindsym $mod+period       exec "$SCRIPT_PATH/scratchpad-temp.sh 3 create-show"
bindsym shift+$mod+period exec "$SCRIPT_PATH/scratchpad-temp.sh 3 detach"
bindsym ctrl+$mod+period  exec "$SCRIPT_PATH/scratchpad-temp.sh 3 destroy"
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
bindsym $alt+$mod+1 exec "$SCRIPT_PATH/vpn-connect.sh lcn toggle"
exec --no-startup-id "rm /tmp/vpn_lcn.tmp"
#+end_src
** Unicamp
#+begin_src conf :noweb-ref vpn
bindsym $alt+$mod+2 exec "$SCRIPT_PATH/vpn-connect.sh unicamp toggle"
exec --no-startup-id "rm /tmp/vpn_unicamp.tmp"
#+end_src
** LBiC
#+begin_src conf :noweb-ref vpn
bindsym $alt+$mod+3 exec "$SCRIPT_PATH/vpn-connect.sh lbic toggle"
exec --no-startup-id "rm /tmp/vpn_lbic.tmp"
#+end_src
** Samsung
#+begin_src conf :noweb-ref vpn
bindsym $alt+$mod+4 exec "$SCRIPT_PATH/vpn-connect.sh samsung toggle"
exec --no-startup-id "rm /tmp/vpn_samsung.tmp"
#+end_src
** Proton VPN
#+begin_src conf :noweb-ref vpn
bindsym $alt+$mod+0 exec "$XDG_CONFIG_HOME/waybar/scripts/proton-vpn.sh toggle"
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
...
*** Sway
**** D-Bus
#+begin_src conf :noweb-ref sway-dbus
# XDG Desktop Portals & systemd user services
exec dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP=sway SWAYSOCK XDG_SESSION_TYPE XDG_CONFIG_HOME
#+end_src
**** Systemd
Deprecated. The "--systemd" in the D-Bus command above already does the trick.
#+begin_src conf :noweb-ref sway-systemd :tangle no
# push env vars to systemd and announce the graphical session
#exec systemctl --user import-environment WAYLAND_DISPLAY XDG_CURRENT_DESKTOP && systemctl --user start graphical-session.target
#+end_src
**** Session
#+begin_src conf :noweb-ref sway-session
# start systemd session target
...
** lan-mouse status
*** Sway
Create file to track lan-mouse connection (used by status bar).
#+begin_src conf :noweb-ref sway-lanmouse
exec echo "disconnected" > /tmp/lanmouse-status.tmp
#+end_src
** xremap mode
Create file to track xremap's mode (used by status bar).
#+begin_src conf :noweb-ref sway-xremap
exec echo "default" > /tmp/xremap_mode.tmp
#+end_src
** KMonad status
(deprecated) +Create file to track KMonad service status (used by status bar).+
...
*** Sway
**** Script
#+begin_src conf :noweb-ref sway-gamma
exec gammastep
#+end_src
**** Status
Create file to track screen color temperature (used by status bar).
#+begin_src conf :noweb-ref sway-gamma
exec echo "6500" > /tmp/gamma_temp.tmp
#+end_src
** Proton VPN
#+begin_src conf :noweb-ref proton-vpn
exec echo "disconnected" > /tmp/proton-vpn-status.tmp
#+end_src
** Waybar
*** Reset color
#+begin_src conf :noweb-ref waybar
exec echo '@define-color dynamic_bg rgba(43, 48, 59, 0.5);' >~/.config/waybar/dynamic-bg.css
#+end_src
*** Dynamic color per output
#+begin_src conf :noweb-ref waybar :tangle no
#exec "$SCRIPT_PATH/waybar-dynamic-color.sh"
#+end_src
** +Output management+
Deprecated. Kanshi is being launched as an user systemd service.
#+begin_src conf :noweb-ref kanshi :tangle no
#exec --no-startup-id kanshi # dynamic automatic output profile management
#+end_src
** Background images
*** i3
...
exec --no-startup-id fcitx5 -d                  # input method framework (cedilla fix)
exec --no-startup-id flashfocus                 # focus animation
exec --no-startup-id autotiling                 # autotiling
#exec --no-startup-id /usr/lib/pam_kwallet_init  # unlock KWallet
...
#exec --no-startup-id "$SCRIPT_PATH/ntf-battery/ntf-battery.sh"     # battery notification
...
exec_always kanshictl reload                    # force kanshi to apply the background wallpapers
#+end_src
*** Config
#+begin_src conf :noweb-ref daemons
...
bindsym $alt+ctrl+l exec "emacsclient -c -s efs"                                    # emacsclient for EFS
#bindsym $alt+ctrl+shift+l exec "emacsclient -c -s doom -a emacs"                    # emacsclient for Doom
bindsym $alt+ctrl+shift+l exec 'emacsclient -s efs --eval "(emacs-everywhere)"'     # emacs-everywhere
...
#+end_src
*** Sway (Waybar)
Deprecated in favour of systemd service.
...
exec brave-origin
exec --no-startup-id "sleep 5 && swaymsg '[app_id=brave-origin]' move to workspace number '2:2' && swaymsg workspace '2:2'"
#+end_src
** Tangle :noexport:
*** i3
#+begin_src conf :noweb yes :tangle _config_i3
